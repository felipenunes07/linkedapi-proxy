...
| F2.29-F2.30 | Preço R$ 67, assento adicional pelo mesmo checkout (grupo de tenants, `seat_token` de uso único, lista de contas e troca no painel), painel em formato de dashboard e domínio próprio na tela de conexão | ✅ **NO AR em 2026-09-13**: migration 0011 aplicada, Worker `00cb07dd`, landing publicada na Vercel (R$ 67 na página e na cobrança), smoke ok. 249 testes, com as correções da revisão de segurança (F2.31). Falta: confirmar o valor do secret `UNIPILE_AUTH_HOST` antes do primeiro onboarding ([ACOES-HUMANAS.md](ACOES-HUMANAS.md) seções 0 e 3) |
