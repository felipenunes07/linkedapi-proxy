...
Snapshot para quem for tocar o projeto de onde paramos. Atualizado em 2026-09-13.
...
Todo o código da V1, da fase 2, do painel do cliente, do cartão recorrente
**e dos assentos adicionais** está **pronto e verde**: typecheck + **243
testes** (18 arquivos),
...
| F2.32 | Painel logado e documentação com o desenho de um console de API, com a nossa marca: barra lateral de altura inteira, seções (Contas, Webhooks, Chaves, Uso, Configurações, Assinatura) com ação principal no topo e listas em linha; `/docs` com barra própria, busca no topo e tema da marca | ✅ código em 2026-09-11, verificado no navegador (desktop e 375px). Sai no mesmo deploy do Worker e push da landing |
| F2.29-F2.30 | Preço R$ 67, assento adicional pelo mesmo checkout (grupo de tenants, `seat_token` de uso único, lista de contas e troca no painel), painel em formato de dashboard e domínio próprio na tela de conexão | ✅ **NO AR em 2026-09-13**: migration 0011 aplicada, Worker `00cb07dd`, landing publicada na Vercel (R$ 67 na página e na cobrança), smoke ok. 249 testes, com as correções da revisão de segurança (F2.31). Domínio próprio também na tela de conexão desde 2026-09-14: certificado emitido pela origem e `UNIPILE_AUTH_HOST` ligado ([ACOES-HUMANAS.md](ACOES-HUMANAS.md) seção 3) |
...
| `supabase/migrations/` | Schema 0001-0011 (`bootstrap.sql` = tudo em um) |
| `test/` | 243 testes (destaques: `isolation`, `connect`, `eventHooks`, `portal`, `checkout`) |
